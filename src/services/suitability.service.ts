...
import { regions, demographics, pois } from '../db/schema.js';
 ...
 */
function competitorPenalty(count: number, maxReasonable: number = 10): number {
 ...
 */
export async function computeScore(request: SuitabilityScoreRequest): Promise<SuitabilityScoreResponse> {
  ...
  const regionResult = await db.execute(
    sql`SELECT id FROM regions WHERE ST_Contains(boundary, ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326)) AND level = 'kelurahan' LIMIT 1`
  );

  const regionRow = (regionResult.rows as Record<string, unknown>[])[0];
  if (!regionRow) {
    throw new AppError(404, 'REGION_NOT_FOUND', 'No kelurahan found for the given coordinates');
  }
  const regionId = regionRow.id as string;

  // 2. Get demographic data
  const yearCondition = dataYear ? sql`AND data_year = ${dataYear}` : sql`ORDER BY data_year DESC`;
  const demoResult = await db.execute(
    sql`SELECT * FROM demographics WHERE region_id = ${regionId} ${yearCondition} LIMIT 1`
  );

  const demo = (demoResult.rows as Record<string, unknown>[])[0];
  if (!demo) {
  ...
  const competitorResult = await db.execute(
    sql`SELECT COUNT(*)::int as count FROM pois WHERE type = 'competitor' AND ST_DWithin(location::geography, ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326)::geography, ${radiusKm * 1000})`
  );
  const competitorCount = ((competitorResult.rows as Record<string, unknown>[])[0]?.count as number) || 0;

  // 4. Compute sub-scores
  const populationScore = normalize(demo.population_density as number, 0, 50000);
  const purchasingScore = (demo.purchasing_power as number) || 0; // Already 0-100
  const accessibilityScore = (demo.accessibility_score as number) || 0; // Already 0-100
  ...
  const rawScore =
    (weights.population / 100) * populationScore +
...
export async function getLocationAnalysis(
  regionId: string,
  params: { radiusKm?: number; dataYear?: number; weights?: SuitabilityWeights },
  ...
  const yearCondition = params.dataYear
    ...
    : sql``;
  const demoResult = await db.execute(
    sql`SELECT * FROM demographics WHERE region_id = ${regionId} ${yearCondition} ORDER BY data_year DESC LIMIT 1`
  );
  const demo = (demoResult.rows as Record<string, unknown>[])[0];
  ...
  if (region.center) {
    ...
    const centerMatch = (region.center as string).match(/POINT\(([\d.-]+)\s+([\d.-]+)\)/);
    if (centerMatch) {
  ...
  const poiCountResult = await db.execute(
    sql`SELECT type, COUNT(*)::int as count FROM pois WHERE ST_DWithin(location::geography, (SELECT center::geography FROM regions WHERE id = ${regionId}), ${radiusMeters}) AND type IN ('competitor', 'office') GROUP BY type`
  );

  const poiCounts = (poiCountResult.rows as Array<{ type: string; count: number }>).reduce(
    (acc, row) => ({ ...acc, [row.type]: row.count }),
    { competitor: 0, office: 0 } as Record<string, number>,
  );
  ...
  return {
    ...
    breakdown,
    demographics: {
      totalPopulation: (demo?.total_population as number) || 0,
      age18_35Pct: (demo?.age_18_35_pct as number) || 0,
      dataYear: (demo?.data_year as number) || 2023,
    },
    radiusAnalysis: {
      radiusKm: params.radiusKm || 1,
