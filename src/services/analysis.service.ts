// Kita buat struktur data untuk menerima nilai slider dari UI
export interface SuitabilityWeights {
  kepadatanPenduduk: number;  // misal: 57
  dayaBeli: number;           // misal: 65
  aksesibilitas: number;      // misal: 90
  kepadatanKompetitor: number; // misal: -40
}

export const analyzeLocationPotential = async (
  lat: number,
  lng: number,
